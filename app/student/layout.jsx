...
export default function StudentLayout({ children }) {
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            {/* Sidebar */}
            <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-indigo-800 transform transition-transform duration-200 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
                }`}>
                <div className="flex items-center justify-between h-16 px-4 bg-indigo-900">
                    <span className="text-xl font-semibold text-white">Student Portal</span>
                    <button
                ...
                <nav className="px-2 py-4">
                    {navItems.map((item) => (
                        <Link
                            key={item.path}
                            href={item.path}
                            className={`flex items-center px-4 py-2 mt-2 text-gray-100 rounded-lg hover:bg-indigo-700 ${pathname === item.path ? 'bg-indigo-700' : ''
                                }`}
                        >
                            {item.name}
                        </Link>
                    ...
                    <div className="flex items-center justify-between h-16 px-4">
                        ...
                        <div className="relative flex items-center">
                            ...
                            <button className="flex items-center text-gray-800 hover:text-gray-600">
                                ...
                                <span className="mr-2">John Student</span>
